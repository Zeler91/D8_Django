...
def index(request):
    ...
    for task in TodoItem.objects.all():
        if task.priority > 1:
            priority_count["высокий приоритет"] = 0
            priority_count["средний приоритет"] = 0
            priority_count["низкий приоритет"] = 0
            break

    for task in TodoItem.objects.all():
        if task.priority == 1:
            priority_count["высокий приоритет"] += 1
        if task.priority == 2:
            priority_count["средний приоритет"] += 1
        if task.priority == 3:
            priority_count["низкий приоритет"] += 1
    ...
    if cache.get("now"):
        ...
        cache.set('now', datetime.datetime.now().strftime('%H:%M %d.%m.%Yг.'), 300)
        now = cache.get('now')

    return render(request, "tasks/index.html", {"counts": counts, "priority_count": priority_count, "today": today, "now": now})
